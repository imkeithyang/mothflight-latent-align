...
def build_sweep_bounds(data, moth_id: int, low_pct: float, high_pct: float, sweep_max: float = None):
    shared = data["spike_latent"][:, : data["shared_latent_dim"]]
...
def sweep_shared_latent(model, ref_record, low_vec, high_vec, steps):
    ...
    for alpha in alphas:
        ...
        cross_flower_mean, cross_flower_recon = model._decode_flower(shared_tensor)
        ft_pred = model._predict_ft(shared_tensor, mass, moth_id)
        ...
        sweep_outputs.append(
            {
                ...
                "flower_recon_mean": cross_flower_mean.detach().cpu().numpy()[0],
                "flower_recon": cross_flower_recon.detach().cpu().numpy()[0],
...
def render_sweep_frame(
    ...
    for row, ref_record in enumerate(ref_records):
        ...
        if flower_recon_mode == "mean":
            ...
            flower_ax.bar(x, flower_vals, color=["tab:blue", "tab:orange"])
            ...
            t = np.arange(out["flower_recon"].shape[0])
            flower_ax.plot(t, out["flower_recon"][:, 0], label="Position", color="tab:blue")
            flower_ax.plot(t, out["flower_recon"][:, 1], label="Velocity", color="tab:orange")
            flower_ax.set_xlabel("Timestep")
            flower_ax.set_ylabel("Value")
            flower_ax.set_ylim(*flower_ylim)
            flower_ax.legend(frameon=False, loc="upper right")
            flower_ax.set_title(f"{row_title}\nflower sequence")
...
def save_moth_sweep_gif(output_dir: Path, moth_id: int, ref_records, sweep_outputs_by_ref, flower_recon_mode: str):
    ...
    for outputs in sweep_outputs_by_ref:
        for out in outputs:
            ...
            if flower_recon_mode == "mean":
                ...
                all_flower.append(out["flower_recon"].reshape(-1))
    ...
    flower_pad = 0.1 * max(1.0, flower_hi - flower_lo)
...
def main():
    ...
    for moth_id in unique_moths:
        ...
        gif_path = save_moth_sweep_gif(output_dir, moth_id, ref_records, sweep_outputs_by_ref, flower_recon_mode)
